...
class Add extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			partial_payment: props.contact.partial_payment,
	...
	UNSAFE_componentWillReceiveProps(nextProps, nextContext) {
		this.setState({
			...
			partial_payment: nextProps.contact.partial_payment,
	...
	render() {
		...
		return (
			<Modal
				title={contact.name === "" ? "Add Agent" : "Update Agent"}
				toggle={onContactClose}
				visible={open}
				closable={false}
				onCancel={onContactClose}
				onOk={() => {
					if (this.state.name === "" || this.state.company === "" || this.state.email === "" || this.state.password === "") {
						return;
					}
					onSaveContact(contact.id, {
						...
						partial_payment: this.state.partial_payment,
					...
					<Col span={17}>
						<Row gutter={10}>
							...
							<Col span={12}>
								<div className="gx-form-group">
									Partial Payment
									<Switch style={{ marginLeft: 16 }} checked={this.state.partial_payment} onChange={e => this.setState({ partial_payment: e })} />
								</div>
							</Col>
						</Row>
						<Row gutter={5}>
							<Col span={this.props.contact.id ? 0 : 8}>
								<Select
									value={this.state.special === true ? "Special Agent" : "Agent"}
									style={{ width: "100%" }}
									placeholder="Type*"
									onChange={event => this.setState({ special: event === "Special Agent" })}
									required>
									<Option value="Agent">Agent</Option>
									<Option value="Special Agent">Group</Option>
								</Select>
							</Col>
							<Col span={this.props.contact.id ? 24 : 16}>
								<div className="gx-form-group">
									<Input
										required
